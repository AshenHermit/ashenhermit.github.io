...
$(document).on('touchstart', '.add-output', function(e) {
	sPrompt("New output name", "").then(function(name){
		addOutputToNode(parseInt($(e.changedTouches[0].target).parent().parent().attr("data-id")),name);
	});
});
...
$(document).on('touchstart', function(e) {
	middleDown = true;
	lastTouchPos.x = e.changedTouches[0].pageX;
	lastTouchPos.y = e.changedTouches[0].pageY;
});

$(document).on('touchstart', '.title', function(e) {
	middleDown = false;
	selectedElement = e.changedTouches[0].target.parentNode.parentNode;
...
$(document).on('touchstart', '.comment', function(e) {
	middleDown = false;
	selectedElement = e.changedTouches[0].target;
	pos.x = parseFloat(selectedElement.style.left);
	pos.y = parseFloat(selectedElement.style.top);
});
...
$(document).on('touchmove', function(e) {
	let movementX = e.changedTouches[0].pageX-lastTouchPos.x;
	let movementY = e.changedTouches[0].pageY-lastTouchPos.y;
	...
	if(middleDown){
		$('.window-container, .comment').toArray().forEach(function(el){
			$(el).css({
				left: el.getClientRects()[0].x+movementX/window.devicePixelRatio,
				top:  el.getClientRects()[0].y+movementY/window.devicePixelRatio
			});
		...
		$(document.body).css({
			backgroundPositionX: parseFloat($(document.body).css('background-position-x'))+movementX/window.devicePixelRatio,
			backgroundPositionY:  parseFloat($(document.body).css('background-position-y'))+movementY/window.devicePixelRatio
		});
	...
	if(selectedElement!=undefined){
		middleDown = false;
		pos.x += movementX/window.devicePixelRatio;
		pos.y += movementY/window.devicePixelRatio;
		$(selectedElement).css({
	...
	if(curve!=undefined){
		middleDown = false;
		var targetPos = {
			x: (e.changedTouches[0].target.className!="inputBox") ? e.changedTouches[0].pageX : e.changedTouches[0].target.getClientRects()[0].x+e.changedTouches[0].target.getClientRects()[0].width/2,
			y: (e.changedTouches[0].target.className!="inputBox") ? e.changedTouches[0].pageY : e.changedTouches[0].target.getClientRects()[0].y+e.changedTouches[0].target.getClientRects()[0].height/2
		}
...
$(document).on('touchstart', '.output-line-box', function(e) {
	middleDown = false;
	console.log(nodes[getOutBoxNodeId(e.changedTouches[0].target)].outputs[getOutBoxId(e.changedTouches[0].target)].curve);
	if(nodes[getOutBoxNodeId(e.changedTouches[0].target)].outputs[getOutBoxId(e.changedTouches[0].target)].curve!=null)
		$(nodes[getOutBoxNodeId(e.changedTouches[0].target)].outputs[getOutBoxId(e.changedTouches[0].target)].curve).remove();
		nodes[getOutBoxNodeId(e.changedTouches[0].target)].outputs[getOutBoxId(e.changedTouches[0].target)].curve = null;

	curve = document.createElementNS('http://www.w3.org/2000/svg',"path");
	curveStart.x = e.changedTouches[0].target.getClientRects()[0].x+e.changedTouches[0].target.getClientRects()[0].width/2;
	curveStart.y = e.changedTouches[0].target.getClientRects()[0].y+e.changedTouches[0].target.getClientRects()[0].height/2;
	...
	selectedOut = e.changedTouches[0].target;
});

$(document).on('touchend', '.inputBox', function(e) {
	middleDown = false;
	if (selectedOut!=undefined) {
		if(parseInt($(e.changedTouches[0].target).parent().attr("data-id")) != getOutBoxNodeId(selectedOut)){

			let nodeId = getOutBoxNodeId(selectedOut);
			let outId = getOutBoxId(selectedOut);

			nodes[nodeId].outputs[outId].curve = curve;
			nodes[nodeId].outputs[outId].outputNodeId = parseInt($(e.changedTouches[0].target).parent().attr("data-id"));
			nodes[parseInt($(e.changedTouches[0].target).parent().attr("data-id"))].inputNodeIds.push(nodeId);
...
function NodesToJSON(){
	...
	let checkNode = function(id,parentObj){
		if(nodes[id]){
        	parentObj[nodes[id].title] = {};
			parentObj[nodes[id].title]["_content"] = html2t(nodes[id].content);
			nodes[id].outputs.forEach(function(out){
				parentObj[nodes[id].title][out.name] = {};
				checkNode(out.outputNodeId,parentObj[nodes[id].title][out.name]);
			});
		}
	}
	
	checkNode(startId,treeObj);
	console.log(treeObj);
}
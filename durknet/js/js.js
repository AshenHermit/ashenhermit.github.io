...
function loadJsonPictures(json) {
	...
	var waiting = setInterval(function(){
		if(j.readyState>3){
		    ...
		    goods.forEach(function(item) {
		    document.getElementsByClassName('content')[0].innerHTML+=
				'<div class="wow animated fast flex-center fadeInUp pictureItem">'+'<img alt="'+ ((item.name!=undefined) ? item.name : "параша какая т")+'" style="max-height: 85vh;max-width: 100vw;" class="shopItemImg" src="'+item.imgUrl+'"></img>'+ ((item.name!=undefined) ? '<h2 class="flex-center animated fadeInDown mb-8 white-text shopItemName ">'+item.name+'</h2>' : "") +((item.description!=undefined) ? '<h6 class="flex-center animated fadeInDown mb-8 white-text text-muted" style="color: rgba(255, 255, 255, 0.5) !important;">'+item.description+'</h6>' : "")+((item.price!=undefined) ? '<div class="d-flex flex-row shopItemBuyBlock"><h5 class="flex-center animated fadeInDown mb-8 white-text text-muted shopItemPrice">'+item.price+'<span style="text-decoration: line-through;">щ</span></h5><div class="animated fadeInDown waves-effect waves-light black-text shopItemButton">заказать</div></div>' : "")+((item.date!=undefined) ? '<h6 class=" animated fadeInDown mb-8 white-text text-muted shopItemDate">'+item.date+'</h6>' : "")+'</div>'
			});
			clearInterval(waiting);
		}
	},100);
